...
interface Props {
  ...
  onChange: (data: { url: string; enabled: boolean }) => void;
}

export const FacebookEdit = ({ data, onChange }: Props) => {
  return (
    <div className="p-4 border border-border rounded-xl bg-gray-50 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-foreground">Facebook Configuration</h3>
      ...
      {data.enabled && (
        <div>
          <label className="block text-sm font-medium text-foreground-muted mb-1">Facebook URL</label>
          <input 
             type="url" value={data.url} onChange={e => onChange({ ...data, url: e.target.value })}
